...
// RefreshTokens and PasswordResetTokens are never deleted anywhere else — every login/refresh/
// reset adds a row and nothing ever removes the ones that have since expired, so both tables
// grow forever. Runs on a timer for the lifetime of the process rather than as a one-shot
// scheduled job, since the app has no external scheduler infrastructure.
